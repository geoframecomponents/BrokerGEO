 ...
 * and the optional per-cell {@code rootDensity}/{@code g} arrays - {@code g}
 * is shared between the transpiration and evaporation splits, since {@code
 * CurrentStepInput} has no separate evaporation-side stress array) - the
 * same {@code ADb}/{@code SqlName}-based convention as WHETGEO-1D's
 * {@code Whetgeo1DInputsHandler} and GEOET's {@code GeoetInputsHandler}.
 *
 */
public class BrokerGeoInputsHandler implements AutoCloseable {
	...
	// NOTE: CurrentStepInput has exactly one "g" array, shared between the
	// transpiration and evaporation splits in ETsBrokerTwoFluxesSolverMain
	// (both calls pass the same `input` object) - there is no per-flux "gE". This single-array
	// assumption is untested for that case - verify it's the intended physics
	// before relying on it with a water-weighted representativeEsModel.
	public double[] g;
